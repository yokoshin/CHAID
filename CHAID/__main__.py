...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run the chaid algorithm on a" " csv/sav file."
    )
    ...
    parser.add_argument(
        "--min-parent-node-size",
        type=int,
        help="Minimum number of " "samples required to split the parent node",
    )
    parser.add_argument(
        "--min-child-node-size",
        type=int,
        help="Minimum number of " "samples required to split the child node",
    )
    ...
    group.add_argument(
        "--export", action="store_true", help="Whether to export the chart to pdf/dot"
    )
    group.add_argument("--export-path", type=str, help="Path to store chart output")

    group.add_argument(
        "--exhaustive", action="store_true", help="To implement exhustive CHAID"
    )
    ...
    types = dict(
        zip(nominal + ordinal, ["nominal"] * len(nominal) + ["ordinal"] * len(ordinal))
    )

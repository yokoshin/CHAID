...
def chisquare(n_ij, weighted):
    ...
    if weighted:
        ...
        m_ij = (np.vstack(n_ij.sum(axis=1)) * n_ij.sum(axis=0)) / n_ij.sum().astype(
            float
        )
    ...
    def best_cat_heuristic_split(self, ind, dep):
        ...
        for i, ind_var in enumerate(ind):
            split.invalid_reason = (
                None  # must reset because using invalid reason to break
            )
            ...
            if dep.weights is None:
                for col in unique:
                    counts = np.unique(
                        np.compress(ind_var.arr == col, dep.arr), return_counts=True
                    )
                ...
                for col in unique:
                    ...
                    for dep_v in all_dep:
                        freq[col][dep_v] = dep.weights[
                            (ind_var.arr == col) * (dep.arr == dep_v)
                        ].sum()
            ...
            while next(ind_var.possible_groupings(), None) is not None:
                ...
                for comb in ind_var.possible_groupings():
                    ...
                    if (
                        choice is None
                        or p_split > highest_p_join
                        or (p_split == highest_p_join and chi > split_chi)
                    ):
                        choice, highest_p_join, split_chi = comb, p_split, chi

                sufficient_split = (
                    not highest_p_join or highest_p_join < self.alpha_merge
                )
                if not sufficient_split:
                    ...
                    temp_split = Split(
                        i, ind_var.groups(), chi, p_split, dof, split_name=ind_var.name
                    )
                    better_split = (
                        not split.valid()
                        or p_split < split.p
                        or (p_split == split.p and chi > split.score)
                    )
            ...
            while next(ind_var.possible_groupings(), None) is not None:
                ...
                for comb in ind_var.possible_groupings():
                    ...
                    if (
                        choice is None
                        or p_split > highest_p_join
                        or (p_split == highest_p_join and score > split_score)
                    ):
                        choice, highest_p_join, split_score = comb, p_split, score

                sufficient_split = highest_p_join < self.alpha_merge and all(
                    len(node_v) >= self.min_child_node_size
                    for node_v in keyed_set.values()
                )
                ...
                if not sufficient_split:
                    ...
                    self.is_exhaustive and len(list(ind_var.possible_groupings())) != 1
                ):
                    ...
                    temp_split = Split(
                        ...
                        p_split,
                        dof,
                        split_name=ind_var.name,
                    )

                    better_split = (
                        not split.valid()
                        or p_split < split.p
                        or (p_split == split.p and score > split.score)
                    )
                ...
                keyed_set[choice[0]] = np.concatenate(
                    (keyed_set[choice[1]], keyed_set[choice[0]])
                )
